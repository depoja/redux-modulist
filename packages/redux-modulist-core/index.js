import { genUID, uniqueArr } from "./utils";
...
const createModule = ({ name = "module", ...features }, custom = []) => {
  ...
  const keysOrder = uniqueArr([...libKeys, ...customKeys, "views"]);
  ...
  const Module = orderedKeys.reduce(
    ...
    { name, instance: name + "_" + genUID() }
  );

  return Module;
};

const Modulist = createModule;
const internalFeatures = getFeatures();
const internalWrappers = getFeatureFn(internalFeatures, "wrapper");

Modulist.actions = internalWrappers.actions;
Modulist.reducer = internalWrappers.reducers;
Modulist.selectors = internalWrappers.selectors;
Modulist.view = internalWrappers.views;
